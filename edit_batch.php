...
// --- GET BATCH ID ---
// Get the batch ID from the URL's query string (e.g., edit_batch.php?id=5).
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_batch'])) {
    ...
    if (!empty($batch_name) && count($course_ids) > 0) {
        ...
        try {
            // Step 1: Update the batch name in the 'batches' table.
            ...
            // This is simpler than checking which ones were added or removed.
        ...
        $error_message = "Batch name cannot be empty and at least one course must be selected.";
    }
}

// --- DATA FETCHING FOR THE FORM ---
// Fetch the current details of the batch being edited to pre-fill the form.
...
$assigned_course_ids = [];
// Store the assigned course IDs in an array for easy lookup later.
while ($row = $assigned_courses_result->fetch_assoc()) {
    $assigned_course_ids[] = $row['course_id'];
}
$assigned_courses_stmt->close();

// Fetch all available courses from the database to populate the selection box.
                ...
                <div class="mb-3">
                    ...
                    <select multiple class="form-select" id="course_ids" name="course_ids[]" size="10" required>
                        <?php while($course = $courses_result->fetch_assoc()): ?>
                            <!-- For each course, check if its ID is in the array of assigned courses. -->
                            <!-- If it is, add the 'selected' attribute to the option tag. -->
